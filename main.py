    ...
    def display_f_arrows(self):
        ...
        for i in range(len(self.bodies)):
            ...
            self.bodies[i].f_arrow = self.ax.quiver(self.bodies[i].position[0], self.bodies[i].position[1],
                                                    ...
                                                    # Calculate length that is proportional in size to force
                                                    # (ratio of force)
                                                    # So it appears on screen in visible size
                                                    length=(np.linalg.norm(self.bodies[i].velocity) / 5197) * (
                                                        self.get__scale_distance()),
                                                    # Define aesthetic of arrow
                                                    normalize=True, linewidth=0.5, arrow_length_ratio=0.1)
...
class Body:
    ...
    def display(self):
        ...
        self.dot, = gui.solar_system.ax.plot([self.position[0]], [self.position[1]], [self.position[2]],
                                             'o', color=self.colour, markersize=5)
...
class GUI:
    ...
    def draw_figure_simulation(self, canvas, figure):
        ...
        self.figure_canvas_simulation.get_tk_widget().pack(side='top', fill='both', expand=2)
        # Returns drawing
    ...
    def start_simulation(self, row_counter, values):
        ...
        # Disables learn button as program is running
        self.window['-LEARN-'].update(disabled=True)
